...
print_results <- function(cmp1, cmp2, param1, param2, n_jobs, w_test, t_test, mean1, mean2, significant_w_test, significant_t_test) {
  cat("\n----------------------------------------------\n")
...
compare_init <- function(init1, init2, pivot, neigh, jobs) {
  ...
  print_results(init1, init2, pivot, neigh, n_jobs, test_results$w_test, test_results$t_test, test_results$mean1, 
...
compare_pivots <- function(pivot1, pivot2, init, neigh, jobs) {
  ...
  test_results <- perform_tests(merged)
  print_results(pivot1, pivot2, init, neigh, n_jobs, test_results$w_test, test_results$t_test, test_results$mean1, 
                test_results$mean2, test_results$significant_w_test, test_results$significant_t_test)
}
...
compare_neighs <- function(neigh1, neigh2, init, pivot, jobs) {
  ...
  test_results <- perform_tests(merged)
  print_results(neigh1, neigh2, init, pivot, n_jobs, test_results$w_test, test_results$t_test, test_results$mean1, 
                test_results$mean2, test_results$significant_w_test, test_results$significant_t_test)
}
...
for (pivot in pivots) {
  for (neigh in neighborhoods) {
    for (n_jobs in n_jobs_arr) {
      compare_init(inits[1], inits[2], pivot, neigh, n_jobs)
    }
  }
}
...
for (init in inits) {
  for (neigh in neighborhoods) {
    for (n_jobs in n_jobs_arr) {
      compare_pivots(pivots[1], pivots[2], init, neigh, n_jobs)
    }
  }
}
...
for (pair in neigh_pairs) {
  for (init in inits) {
    for (pivot in pivots) {
      for (n_jobs in n_jobs_arr) {
        compare_neighs(pair[1], pair[2], init, pivot, n_jobs)
      }
    }
  }
}
