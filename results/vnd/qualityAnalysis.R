...
print_results <- function(cmp1, cmp2, w_test, t_test, mean1, mean2, significant_w_test, significant_t_test) {
  cat("\n----------------------------------------------\n")
...
compare_init <- function(strategy1, strategy2) {
  a1 <- results %>% filter(neighborhood_order==strategy1)  # Selects only the lines that respect the conditions
  a2 <- results %>% filter(neighborhood_order==strategy2)
  ...
  print_results(strategy1, strategy2, test_results$w_test, test_results$t_test, test_results$mean1, 
